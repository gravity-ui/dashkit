...
export default class GridLayout extends React.PureComponent {
    ...
    updateDraggingElementState(group, layoutItem, e) {
        ...
        if (!currentDraggingElement) {
            ...
            const items = [
                ...this.context.config.items,
                ...(this.context.config.globalItems || []),
            ];

            const item = temporaryLayout
                ? temporaryLayout.dragProps
                : items.find(({id}) => id === layoutId);
    ...
    render() {
        ...
        const items = [...this.context.config.items, ...(this.context.config.globalItems || [])];

        this.pluginsRefs.length = items.length;
        ...
        const itemsByGroup = items.reduce((memo, item) => {
