...
import {
    DashKitContext,
    type DashKitCtxShape,
    DashKitDnDContext,
    DashkitOvelayControlsContext,
    type OverlayControlsCtxShape,
...
function useMemoStateContext(props: DashKitWithContextProps): UseMemoStateContextResult {
    ...
    const getItemsMeta = React.useCallback((pluginsRefs: PluginRef[]) => {
        return pluginsRefs
            .map((ref) => {
    ...
    const reloadItems = React.useCallback(
        (pluginsRefs: PluginRef[], data: {silentLoading: boolean; noVeil: boolean}) => {
            pluginsRefs.forEach((ref) => {
                if (ref && hasReload(ref)) {
                    ref.reload(data);
                }
            });
        },
    ...
    const onDropDragOver = React.useCallback(
        (
            ...
            sharedItem: DraggedOverItem | undefined,
        ) => {
            if (temporaryLayout) {
                resetTemporaryLayout();
                return false;
            ...
            const maxH = Math.min(
                ...
                ('maxH' in defaultLayout ? defaultLayout.maxH : undefined) || Infinity,
            );
            ...
            if (
                maxW === 0 ||
                maxH === 0 ||
                maxW < (('minW' in defaultLayout ? defaultLayout.minW : undefined) || 0) ||
                maxH < (('minH' in defaultLayout ? defaultLayout.minH : undefined) || 0)
            ) {
                return false;
            }
    ...
    const onDrop = React.useCallback(
        (newLayout: ConfigLayout[], item: ConfigLayout) => {
            if (!dragPropsContext) {
                return;
            }
