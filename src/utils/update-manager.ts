...
function removeItemVersion1({id, config, itemsStateAndParams}: RemoveItemArg) {
    ...
    const removeItem = items[itemIndex];
    ...
    const allParamsKeys = getParams(id, items);
    const allNamespaceParamsKeys = getParams(
        id,
        items.filter((item) => item.namespace === removeItem.namespace),
    );
    ...
    const withoutUniqItemsParams = Object.keys(itemsStateAndParams)
        ...
        .reduce(
            (acc, key) => {
                ...
                const item = items.find((configItem) => configItem.id === key);
                if (params && item) {
